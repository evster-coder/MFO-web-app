...
@section('content')
	...
	<x-auth-session-status class="mb-4" :status="session('status')" />
  	<x-auth-validation-errors class="mb-4" :errors="$errors" />
      	...
      	<table class="table table-bordered data-table" id="data-table" data-url="/loanterms/get-loanterms">
            <thead>
            ...
            <tbody>
              <x-loanterms-tbody :terms="$terms"></x-loanterms-tbody>
            </tbody>
      	</table>
          ...
          <div class="modal-dialog">
            ...
            <h4 class="modal-title" id="dataCrudModal"></h4>
              ...
              <div class="modal-body">
                <form name="dataForm" id="dataForm" action="{{ route('loanterm.store') }}" method="POST">
                  <input type="hidden" name="dataId" id="dataId" >
                  @csrf
                  <div class="row">
                    <div class="form-group edit-fields">
                      <label for="daysAmount">Количество дней</label>
                      <input type="number" name="daysAmount" id="form-data" class="form-control" placeholder="Количество дней" oninput="validate()" >
                    </div>
                    <div class="btn-group block-padding">
                      ...
                      <a href="{{ route('loanterm.index') }}" class="btn btn-danger">Отмена</a>
                    </div>
                  </div>
                </form>
              </div>
