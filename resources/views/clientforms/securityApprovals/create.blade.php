...
@section('content')
	<a href="{{route('securityApproval.tasks')}}" class="btn btn-default">< К списку</a>
	...
	<div class="content-block">
    	...
    	<div class="block-padding d-flex">
            ...
            <form method="POST" action="{{route('securityApproval.accept')}}"
            id="accept">
                @csrf
            ...
            <form method="POST" action="{{route('securityApproval.reject')}}"
             id="reject">
                @csrf
                <input type="hidden" name="clientform_id" value="{{$clientform->id}}">
                <input type="hidden" name="comment">
    	...
    	<div class="block-section">
    		<h4>Клиент</h4>
    		<a target="_blank" href="{{route('client.show', 
            ['id' => $clientform->client_id])}}" class="btn btn-info">
    			Проверка клиента
    		</a>
		  	<hr>
            <table class="table">
    	...
    	<div class="block-section">
    		<h4>Анкета</h4>
    		<a target="_blank" href="{{route('clientform.show', 
            ['id' => $clientform->id])}}" class="btn btn-info">
    			Проверка анкеты
    		</a>
    		<hr>
            <table class="table">
