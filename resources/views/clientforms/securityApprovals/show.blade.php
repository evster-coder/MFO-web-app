            ...
            @if($clientForm->securityApproval)
                ...
                <table class="table">
                    ...
                    <tr>
                        <td>Статус</td>
                        <td>
                            {{$clientForm->securityApproval->approval ? "Одобрено" : "Отклонено"}}
                        </td>
                    </tr>
                    <tr>
                        <td>Пользователь</td>
                        <td>
                            <a target="_blank"
                               href="{{route('user.show', ['id' => $clientForm->securityApproval->user->id])}}">
                                {{$clientForm->securityApproval->user->username}}
                                ({{$clientForm->securityApproval->user->full_name}})
                            </a>
                        </td>
                    </tr>
                    <tr>
                        <td>Дата оформления одобрения</td>
                        <td>{{date(config('app.datetime_format', 'd-m-Y H:i:s'),
                                strtotime($clientForm->securityApproval->approval_date))}}</td>
                    </tr>
                    <tr>
                        <td>Комментарий пользователя</td>
                        <td>{{$clientForm->securityApproval->comment}}</td>
                    </tr>
                    </tbody>
                </table>
            @endif
